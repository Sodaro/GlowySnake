...
{
    //Graph with neighbors, nodes on edges are connected with other side
    ...
    */
    ...
    {
        //TODO: Implement failsafe incase all tiles are occupied (wont happen ingame but still)
        do
        {
            row = Random.Range(0, tiles.GetLength(0));
            col = Random.Range(0, tiles.GetLength(0));
        } while (tiles[row, col].content != null);

        return tiles[row, col].position;
    }

    public void SetTileContent(GameObject gameObj, int row, int col)
    {
        if (row < 0 || row > tiles.GetLength(0) || col < 0 || col > tiles.GetLength(1))
            return;

        tiles[row, col].content = gameObj;
    }
